...
async function discoverLoc(source: DirectorResearchSourceRecord) {
  const url = new URL("https://www.loc.gov/photos/");
  url.searchParams.set("fo", "json"); url.searchParams.set("c", String(MAX_ITEMS));
  url.searchParams.set("q", "street vendors laborers clothing");
  const root = record(await fetchJson(url));
  return (Array.isArray(root.results) ? root.results : []).map((value) => locItem(value, source, "collection-item")).filter((value): value is NormalizedEvidenceInput => Boolean(value)).slice(0, MAX_ITEMS);
}
...
async function discoverMet(source: DirectorResearchSourceRecord) {
  const search = new URL("https://collectionapi.metmuseum.org/public/collection/v1/search");
  search.searchParams.set("hasImages", "true"); search.searchParams.set("dateBegin", "1603"); search.searchParams.set("dateEnd", "1868"); search.searchParams.set("geoLocation", "Japan"); search.searchParams.set("q", "kimono");
  ...
  return objects.map((value): NormalizedEvidenceInput | null => {
    ...
    return {
      ...
      culturallySensitive: /funerary|sacred|ceremonial|reliquary|human remains|mask/i.test(`${first(item.title)} ${first(item.objectName)} ${first(item.classification)}`),
    };
...
async function discoverSmithsonian(source: DirectorResearchSourceRecord) {
  const url = new URL("https://api.si.edu/openaccess/api/v1.0/search");
  const configuredKey = process.env.SMITHSONIAN_API_KEY?.trim();
  url.searchParams.set("api_key", configuredKey || "DEMO_KEY"); url.searchParams.set("q", "daily life clothing tools"); url.searchParams.set("rows", String(MAX_ITEMS)); url.searchParams.set("row_group", "objects");
  let payload: unknown;
...
async function discoverEuropeana(source: DirectorResearchSourceRecord) {
  const url = new URL("https://api.europeana.eu/record/v2/search.json");
  const configuredKey = process.env.EUROPEANA_API_KEY?.trim();
  url.searchParams.set("query", "daily life clothing work"); url.searchParams.set("rows", String(MAX_ITEMS)); url.searchParams.set("profile", "rich"); url.searchParams.set("media", "true"); url.searchParams.set("reusability", "open");
  const root = record(await fetchJson(url, { "X-Api-Key": configuredKey || "api2demo" }));
...
async function discoverDpla(source: DirectorResearchSourceRecord) {
  const url = new URL("https://api.dp.la/v2/items"); url.searchParams.set("api_key", requireKey("DPLA_API_KEY")); url.searchParams.set("q", "daily life clothing work"); url.searchParams.set("page_size", String(MAX_ITEMS));
  const root = record(await fetchJson(url));
...
export async function discoverDirectorEvidence(source: DirectorResearchSourceRecord) {
  if (/Library of Congress structured era-evidence/i.test(source.title)) return discoverLoc(source);
  if (/Met Collection API material-world/i.test(source.title)) return discoverMet(source);
  if (/Smithsonian Open Access collections/i.test(source.title)) return discoverSmithsonian(source);
  if (/Europeana Search, Record/i.test(source.title)) return discoverEuropeana(source);
  if (/Digital Public Library of America/i.test(source.title)) return discoverDpla(source);
