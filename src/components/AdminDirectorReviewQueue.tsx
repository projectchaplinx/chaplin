...
export default function AdminDirectorReviewQueue({ initialBundle }: { initialBundle: DirectorResearchBundle }) {
  ...
  const queue = useMemo(() => buildDirectorReviewQueue(studies, analyses), [studies, analyses]);
  const selected = queue.find((item) => item.id === selectedId) ?? queue[0] ?? null;

  const refresh = useCallback(async () => {
    const [researchResponse, timedResponse] = await Promise.all([
      fetch("/api/admin/director-brain", { cache: "no-store" }),
      fetch("/api/admin/director-brain/timed-media", { cache: "no-store" }),
    ]);
    const [research, timed] = await Promise.all([researchResponse.json(), timedResponse.json()]);
        ...
        <div className="grid min-h-[520px] xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="max-h-[680px] overflow-y-auto border-r border-line p-3">
            {queue.map((item, index) => (
              <button key={item.id} type="button" onClick={() => select(item.id)} className={`mb-2 w-full rounded-lg border p-3 text-left ${item.id === selected.id ? "border-accent bg-accent/[0.08]" : "border-white/10"}`}>
                <span className="text-[8px] font-semibold uppercase tracking-[0.13em] text-accent-secondary">{String(index + 1).padStart(2, "0")} · {item.kind === "playback" ? "Playback gate" : "Knowledge gate"}</span>
                <span className="mt-1 block text-xs font-semibold text-ink">{item.analysis?.workTitle || item.study?.studyTitle}</span>
          ...
          <div className="min-w-0 p-4 sm:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-accent-secondary">{selected.kind === "playback" ? "Gate 1 · verify exact passage" : "Gate 2 · decide reusable knowledge"}</p><h3 className="mt-1 text-xl font-semibold text-ink">{selected.analysis?.workTitle || selected.study?.studyTitle}</h3><p className="mt-1 text-[10px] text-grey">{selected.study?.source.institution || "Library of Congress"} · {selected.study?.periodLabel || "Timed film evidence"}</p></div>
